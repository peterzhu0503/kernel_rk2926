...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
	...
	/*check mmc dec_pin -- emmc/nandflash*/
//	if(1)
	if(board_boot_media())
	{
		pdata->det_pin_info.io =RK2928_PIN2_PA3;
		printk("%s: emmc dect is A3 \n",__FUNCTION__);
	}
	else
	{
		pdata->det_pin_info.io = RK2928_PIN2_PA7;
		printk("%s: flash dect is A7 \n",__FUNCTION__);	
	}
