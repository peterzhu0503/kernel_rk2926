...
static int batt_table[2*BATT_NUM+6] =
{
	...
	3526, 3618, 3669, 3710, 3755, 3797, 3851, 3912, 3941, 3989, 4030,  //discharge
...
//xuj add
	...
	if( 1 == bat -> ac_charging ) {
//		if( 0 == gpio_get_value(DC_DET_PIN) )
			gpio_set_value(DC_DET_PIN, 1);
	} else {
//		if( 1 == gpio_get_value(DC_DET_PIN) )
			gpio_set_value(DC_DET_PIN, 0);
	}
//	printk("xuj, gpio_get_value(DC_DET_PIN) = %d, %s %d\n", gpio_get_value(DC_DET_PIN), __func__, __LINE__);
#endif
//xuj end	
	//if (rk30_battery_dbg_level){
		if (++AdcTestCnt >= 2)
...
#if defined(CONFIG_MACH_RK2926_PHONEPAD)
