...
static int boot_media;
static void __init rk2928_boot_media_init(void)
{
	u32 reg_flag = (readl_relaxed(GRF_GPIO1D_IOMUX));  //RK2928_GRF_BASE+0x00c4
	boot_media = 0;      // boot from nand flash
    if((reg_flag & 0x0000FFFF) != 0x00005555)
	    boot_media = 1;  //boot from emmc or sd
    printk("Boot media: %d \n", boot_media);
}

//return 0 is boot from nand flash,return 1 is boot from emmc or sd
int board_boot_media(void)
{
	return boot_media;
}
EXPORT_SYMBOL(board_boot_media);
...
//	clk_disable_unused();
   ...
   rk2928_boot_media_init(); //add zyf 
	rk2928_iomux_init();
